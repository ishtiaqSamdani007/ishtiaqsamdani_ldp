...
MenuCardList.forEach((menuCard) => {
    ...
    <div class="menu-card" draggable="true" attr-key=${menuCard.id}>
        ...
        <span class="show-edit" attr-key=${menuCard.id}>edit</span>
                            ...
                            <p attr-key=${menuCard.id}>Description: <span id="item-desc" attr-key=${menuCard.id}>${menuCard.desc}</span></p>
                        </div>
    `;
...
addBtn.addEventListener("click", () => {
    ...
    addMenuItemBtn.addEventListener("click", (e) => {
        ...
        <div class="menu-card" draggable="true" attr-key=${MenuCardList.length - 1}>
                            <button class="edit"  attr-key=${MenuCardList.length - 1}><img src="./assests/edit.svg" alt="edit-button" class="edit-img edit" attr-key=
                            ${MenuCardList.length - 1}
        <span class="show-edit" attr-key=${MenuCardList.length - 1}edit</span></button>
                            ...
                            <h2 attr-key=${MenuCardList.length - 1}>${newMenuItem.name
            }</h2>
                            <p attr-key=${MenuCardList.length - 1
            }>Total cost: <span id="total-cost" attr-key=${MenuCardList.length - 1
            }>${newMenuItem.cost}</span></p>
                            <p attr-key=${MenuCardList.length - 1
            }>Item type: <span id="item-type" attr-key=${MenuCardList.length - 1
            }>${newMenuItem.type}</span></p>
                            <p attr-key=${MenuCardList.length - 1
            }>Description: <span id="item-desc" attr-key=${MenuCardList.length - 1
            }>${newMenuItem.desc}</span></p>
                        </div>
        `;
        menuCards.insertAdjacentHTML("beforeend", menuCardHtml);
        filterCardsOnAdd();
...
addGlobalEventListener("click", ".edit", (e) => {
    ...
    <form>
        ...
        <br>
        <label for="item-name">Item Name</label>
        <input type="text" name="item-name" id="item-name" placeholder="Item Name" value="${MenuCardList[key].name
        }">
        <br>
        <label for="item-desc">Item Description</label>
        ...
        <select name="item-type" id="item-typee">
            <option value="">--select item type--</option>
            <option value="entree" ${type == "entree" ? "selected" : ""
        }>entree</option>
            <option value="mainCourse" ${type == "mainCourse" ? "selected" : ""
        }>main course</option>
            <option value="desserts"  ${type == "desserts" ? "selected" : ""
        }>desserts</option>
            <option value="appetizers" ${type == "appetizers" ? "selected" : ""
        }>appetizers</option>
            <option value="beverages" ${type == "beverages" ? "selected" : ""
        }>beverages</option>
        </select>
        <br>
        <br>
    ...
    addMenuItemBtn.addEventListener("click", (e) => {
        ...
        const menuCardHtml = `
                            ...
                            <p attr-key="${key}">Description: <span id="item-desc" attr-key="${key}">${newMenuItem.desc}</span></p>
        `;
